...
class ACHoldingChip8:
    ...
    def setup_gui(self):
        ...
        file_menu.add_command(label="Save State to File...", command=self.save_state_file)
        file_menu.add_command(label="Load State from File...", command=self.load_state_file)
        file_menu.add_separator()
        ...
        controls_menu.add_command(label="Pause/Resume", command=self.toggle_pause)
    ...
    def emulation_loop(self):
        ...
        if not self.paused:
            ...
            # Keep timer accumulator from jumping after long pauses.
            self._last_timer_tick = time.perf_counter()
    ...
    def save_state_file(self):
        path = filedialog.asksaveasfilename(
            defaultextension=".ch8state",
            filetypes=[("Chip-8 State", "*.ch8state")],
        )
        if path:
            import pickle

            with open(path, "wb") as f:
                pickle.dump(self.chip8.save_state(), f)
            self.status_var.set("State saved to file")

    def load_state_file(self):
        path = filedialog.askopenfilename(filetypes=[("Chip-8 State", "*.ch8state")])
        if path:
            import pickle
            ...
            self.chip8.draw_flag = True
            self.status_var.set("State loaded from file")
